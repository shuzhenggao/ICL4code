...
for pos in tqdm(range(len(querys))):
    ...
    while success!=1:
        try:
            response = openai.Completion.create(model="text-davinci-003",prompt=query['prompt'],temperature=0,max_tokens=256,top_p=1,frequency_penalty=0.0,presence_penalty=0.0,stop=["\n\n"])
            success=1
            result = {}
